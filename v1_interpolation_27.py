...
def line_to_point(quaternion, title="demo", show=False):
    ...
    data_plot = np.zeros(shape=[frames_clip_length, 27, 3])
    chains = chains27
    for frame in range(frames_clip_length):
        ...
        for chain in chains:
            for i in range(1, len(chain)):
                ...
                if chain[i] == 16:
                    data_plot[frame][17] = data_plot[frame][14]
                    data_plot[frame][22] = data_plot[frame][14]
                count = count + 1
    return data_plot


def interpolation_between_two_action27(file_name_1, file_name_2, file_path='./data/h3.6m/Train/train_xyz/',
                                       model='default'):
    data = {}
    ...
    # ————————————————初始动作————————————————
    data_slerp[str(files[0])] = line_to_point(data_quaternion[files[0]])
    #  ———————————————————————————————————————
    # 两组动作之间插值
    temp1 = []
    ...
    # ———————————————————————————————————————
    plot = plot_human(data_slerp, model=model, save_path='./result_27/')
    plot.plot()
    return item_shape[0], item_shape[1]
